  ...
  if (usage === "inbox") {
    process.stdout.write(
      [
        ...
        "",
        "Subcommands:",
        "  create [--mailbox-name <name>] [--display-name <sender name>] [--domain <domain>] [--webhook-url <url>]",
        "  spawn  [--mailbox-name <name>] [--display-name <sender name>] [--domain <domain>] [--webhook-url <url>] [--key-name <name>]",
        "  list [--limit <n>] [--offset <n>]",
        "  get --id <inbox_id>",
        "  delete --id <inbox_id>",
        "  keys create --id <inbox_id> [--name <name>]",
        "  keys list --id <inbox_id>",
        "  keys revoke --id <inbox_id> --key-id <key_id>",
        "",
        "spawn creates an inbox and mints an inbox-scoped API key for it in one",
        "step. Run it with a pod-scoped key: the inbox lands in that pod, and the",
        "returned apiKey.token can only read/send from that inbox. The token is",
        "shown once and never retrievable again. If minting fails, the inbox is",
        "deleted. Use --json to capture the result programmatically.",
        "",
        "keys manages inbox-scoped API keys for an existing inbox. Needs an",
        "account-wide or pod-scoped key; inbox-scoped keys cannot mint keys.",
        "",
        "Examples:",
        "  openmail inbox spawn --display-name \"Research agent\" --json",
