  ...
  if (usage === "inbox") {
    process.stdout.write(
      [
        ...
        "",
        "keys: inbox-scoped API keys are confined to that one inbox — read, send,",
        "and look up its parent pod; nothing outside it. Needs an account-wide or",
        "pod-scoped key; inbox-scoped keys cannot mint keys. The token is shown",
