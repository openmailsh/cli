  ...
  if (usage === "policy") {
    process.stdout.write(
      [
        ...
        "A pod-scoped key may only change its own pod and its inboxes. It cannot",
        "set `none`, and when a parent scope is in allowlist mode it cannot add",
        "allow rules at all — only block rules to tighten. Inbox keys cannot",
